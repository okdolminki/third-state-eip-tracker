---
meeting_number: 204
source: "ethereum/pm AllCoreDev EL"
upgrade: Pectra, Fusaka

note_type: allcoredevs
auto_generated: true
source_date: 2026-06-01
sources:
  - "https://github.com/ethereum/pm/tree/master/AllCoreDevs-EL-Meetings"
---
# AllCoreDev EL Meeting #204

## 요약
이더리움 코어 개발자 회의 #204에서는 Pectra Devnet 5의 Nethermind 클라이언트 버그 해결과 Devnet 6 출시 준비 상황이 논의되었습니다. Geth 1.14 이상 버전에서 P2P DoS 보안 버그가 발견되어 즉시 업데이트가 권고되었으며, 퍼블릭 테스트넷 업그레이드 슬롯 결정은 Devnet 6 출시 이후로 연기되었습니다. Pectra 시스템 계약에 대한 보안 감사가 완료되었고, 차기 하드포크인 Fusaka에 EOF와 PeerDAS EIP를 포함하기로 결정했습니다. 개발자들은 Pectra Devnet 6 출시에 집중하고, Fusaka 업그레이드에 새로운 EIP를 추가하기 전에 기존 EIP 구현을 우선시하기로 합의했습니다.

## 결정사항
- Nethermind 클라이언트의 BLS 사전 컴파일 최적화 버그가 해결되어 Pectra Devnet 6 출시의 걸림돌이 아님.
- Pectra Devnet 6를 위한 새로운 하이브 릴리스가 준비되었으며, 시스템 계약 주소는 EIP 관련성을 강조하도록 업데이트될 예정.
- 개발자들은 Pectra Devnet 6 출시까지 추가로 일주일 기다린 후, 퍼블릭 테스트넷 업그레이드를 위한 특정 슬롯 번호를 결정하기로 함.
- Geth 1.14 이상 버전을 실행하는 노드는 P2P DoS 버그 수정을 위해 즉시 최신 버전으로 업데이트해야 함.
- 클라이언트 팀은 Pectra Devnet 6 출시에 집중하고, 다음 주 ACD 회의에 EL 및 CL 클라이언트 팀 모두 참석하여 퍼블릭 테스트넷 슬롯 번호를 결정할 준비를 할 것.
- Fusaka 업그레이드 계획 시, 기존 EIP(EOF, PeerDAS)가 멀티 클라이언트 개발넷에 구현될 때까지 새로운 EIP 추가를 자제하기로 함.
- Fusaka 개발넷 일정을 잡기 전에 Pectra Devnet 6 출시에 먼저 집중하기로 함.

## EIP 논의
- **EIP-7793** — 새로운 사전 컴파일 제안 (암호화된 멤풀 지원): *계속 논의*
- **EIP-7843** — 새로운 사전 컴파일 제안 (현재 블록 슬롯 번호 반환): *계속 논의*
- **EIP-7870** — 이더리움 노드 하드웨어 및 대역폭 권장 사항 공식화: *정보 공유*
- **EOF** — EVM Object Format: *포함 결정*
- **PeerDAS** — Peer Data Availability Sampling: *포함 결정*

## 타임라인 변경
퍼블릭 테스트넷 활성화 시점 결정이 Pectra Devnet 6 출시 이후로 일주일 연기되었으며, Fusaka 개발넷 일정 또한 Pectra Devnet 6 출시에 집중하기 위해 연기되었습니다.
