---
eip_number: 8062
title: "Add sweep withdrawal fee for 0x01 validators"
proposal_type: EIP
category: Core
type: "Standards Track"
status: Draft
prev_status: none
change_type: new_draft
author: "Anders Elowsson (@anderselowsson), Toni Wahrstätter (@nerolation), Francesco D'Amato (@fradamt), Ben Adams (@benaadams), Maria Inês Silva (@misilva73)"
created: 2025-10-28
requires:
  - EIP-4895
  - EIP-7251
related_protocols:
  - Ethereum
  - Uniswap
  - Aave
  - Ether.fi
  - Lido
note_type: eip_tracking
detected_at: 2026-04-30T00:41:41.381Z
last_updated: 2026-04-30
---
# EIP-8062: Add sweep withdrawal fee for 0x01 validators

**상태:** 🟡 `Draft`

## 기본 정보

| 항목 | 내용 |
|------|------|
| 번호 | EIP-8062 |
| 타입 | EIP — Standards Track |
| 카테고리 | Core |
| 현재 상태 | Draft |
| 작성자 | Anders Elowsson (@anderselowsson), Toni Wahrstätter (@nerolation), Francesco D'Amato (@fradamt), Ben Adams (@benaadams), Maria Inês Silva (@misilva73) |
| 생성일 | 2025-10-28 |
| 의존 EIP | [[EIP-4895]], [[EIP-7251]] |
| 원문 | [GitHub](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-8062.md) |

## 변화 컨텍스트

> **신규 Draft 채택** — 2026-04-30에 감지됨

## 리서치 노트

_← 여기에 분석 내용을 작성하세요_

## 관련 프로토콜

- [[Ethereum]]
- [[Uniswap]]
- [[Aave]]
- [[Ether.fi]]
- [[Lido]]

## 관련 노트

_← 관련 EIP, 리포트, 거버넌스 노트를 링크하세요_
