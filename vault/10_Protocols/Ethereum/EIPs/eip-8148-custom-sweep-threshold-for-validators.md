---
eip_number: 8148
title: "Custom sweep threshold for validators"
proposal_type: EIP
category: Core
type: "Standards Track"
status: Draft
prev_status: none
change_type: new_draft
author: "Dmitry Gusakov (@dgusakov), Dmitry Chernukhin (@madlabman), Greg Koumoutsos (@gkoumout)"
created: 2026-02-05
requires:
  - EIP-7251
  - EIP-7685
  - EIP-7732
related_protocols:
  - Ethereum
  - Ether.fi
  - Lido
note_type: eip_tracking
detected_at: 2026-04-30T00:41:41.381Z
last_updated: 2026-04-30
---
# EIP-8148: Custom sweep threshold for validators

**상태:** 🟡 `Draft`

## 기본 정보

| 항목 | 내용 |
|------|------|
| 번호 | EIP-8148 |
| 타입 | EIP — Standards Track |
| 카테고리 | Core |
| 현재 상태 | Draft |
| 작성자 | Dmitry Gusakov (@dgusakov), Dmitry Chernukhin (@madlabman), Greg Koumoutsos (@gkoumout) |
| 생성일 | 2026-02-05 |
| 의존 EIP | [[EIP-7251]], [[EIP-7685]], [[EIP-7732]] |
| 원문 | [GitHub](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-8148.md) |

## 변화 컨텍스트

> **신규 Draft 채택** — 2026-04-30에 감지됨

## 리서치 노트

_← 여기에 분석 내용을 작성하세요_

## 관련 프로토콜

- [[Ethereum]]
- [[Ether.fi]]
- [[Lido]]

## 관련 노트

_← 관련 EIP, 리포트, 거버넌스 노트를 링크하세요_
